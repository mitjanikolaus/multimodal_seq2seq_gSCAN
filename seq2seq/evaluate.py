...
def evaluate(data_iterator: Iterator, model: nn.Module, lm_vocab:dict, max_decoding_steps: int, pad_idx: int, sos_idx: int,
             eos_idx: int, max_examples_to_evaluate=None, dataset=None, test_batch_size=1) -> Tuple[float, float, float]:
    accuracies = []
    target_accuracies = []
    lm_losses = []
    exact_match = 0
    for input_sequence, _, _, output_sequence, target_sequence, _, _, aux_acc_target, lm_loss in predict(
    ...
    #get perplexity from losses
    lm_perplexity = torch.exp(torch.mean(torch.Tensor(lm_losses)))
    return (float(np.mean(np.array(accuracies))), (exact_match / len(accuracies)) * 100,
            float(np.mean(np.array(target_accuracies))), lm_perplexity)
